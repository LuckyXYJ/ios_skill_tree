...
#import "XZPointParticleEffect.h"
#import "XZVertexAttribArrayBuffer.h"
//用于定义粒子属性的类型
typedef struct
{
...
}XZParticleAttributes;

//GLSL程序Uniform 参数
enum
{
    XZMVPMatrix,//MVP矩阵
    XZSamplers2D,//Samplers2D纹理
    XZElapsedSeconds,//耗时
    XZGravity,//重力
    XZNumUniforms//Uniforms个数
};

//属性标识符
typedef enum {
    XZParticleEmissionPosition = 0,//粒子发射位置
    XZParticleEmissionVelocity,//粒子发射速度
    XZParticleEmissionForce,//粒子发射重力
    XZParticleSize,//粒子发射大小
    XZParticleEmissionTimeAndLife,//粒子发射时间和寿命
} XZParticleAttrib;

@interface XZPointParticleEffect()
{
    GLfloat elapsedSeconds;//耗时
    GLuint program;//程序
    GLint uniforms[XZNumUniforms];//Uniforms数组
}

//顶点属性数组缓冲区
@property (strong, nonatomic, readwrite)XZVertexAttribArrayBuffer  * particleAttributeBuffer;
...
@implementation XZPointParticleEffect
...
//获取粒子的属性值
- (XZParticleAttributes)particleAtIndex:(NSUInteger)anIndex
{
    
    //bytes:指向接收者内容的指针
    //获取粒子属性结构体内容
    const XZParticleAttributes *particlesPtr = (const XZParticleAttributes *)[self.particleAttributesData bytes];
...
//设置粒子的属性
- (void)setParticle:(XZParticleAttributes)aParticle
            atIndex:(NSUInteger)anIndex
{
    //mutableBytes:指向可变数据对象所包含数据的指针
    //获取粒子属性结构体内容
    XZParticleAttributes *particlesPtr = (XZParticleAttributes *)[self.particleAttributesData mutableBytes];
...
{
    //创建新的例子
    XZParticleAttributes newParticle;
    ...
    for(int i = 0; i < count && !foundSlot; i++)
    {
        ...
        //获取当前旧的例子
        XZParticleAttributes oldParticle = [self particleAtIndex:i];
...
- (NSUInteger)numberOfParticles;
{
    static long last;
    //总数据/粒子结构体大小
    long ret = [self.particleAttributesData length] / sizeof(XZParticleAttributes);
...
- (void)prepareToDraw
{
    ...
    if(program != 0)
    {
        ...
        */
        ...
        glUniformMatrix4fv(uniforms[XZMVPMatrix], 1, 0,modelViewProjectionMatrix.m);
         ...
         */
        glUniform1i(uniforms[XZSamplers2D], 0);
         ...
         */
        glUniform3fv(uniforms[XZGravity], 1, self.gravity.v);
        
        //耗时
        glUniform1fv(uniforms[XZElapsedSeconds], 1, &elapsedSeconds);
        ...
        //准备顶点数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:XZParticleEmissionPosition
         numberOfCoordinates:3
         attribOffset:
         offsetof(XZParticleAttributes, emissionPosition)
         shouldEnable:YES];
        
        //准备粒子发射速度数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:XZParticleEmissionVelocity
         numberOfCoordinates:3
         attribOffset:
         offsetof(XZParticleAttributes, emissionVelocity)
         shouldEnable:YES];
        
        //准备重力数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:XZParticleEmissionForce
         numberOfCoordinates:3
         attribOffset:
         offsetof(XZParticleAttributes, emissionForce)
         shouldEnable:YES];
        
        //准备粒子size数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:XZParticleSize
         numberOfCoordinates:2
         attribOffset:
         offsetof(XZParticleAttributes, size)
         shouldEnable:YES];
        
        //准备粒子的持续时间和渐隐时间数据
        [self.particleAttributeBuffer
         prepareToDrawWithAttrib:XZParticleEmissionTimeAndLife
         numberOfCoordinates:2
         attribOffset:
         offsetof(XZParticleAttributes, emissionTimeAndLife)
         shouldEnable:YES];
...
- (BOOL)loadShaders
{
    ...
    vertShaderPathname = [[NSBundle mainBundle] pathForResource:
                          @"XZPointParticleShader" ofType:@"vsh"];
    ...
    fragShaderPathname = [[NSBundle mainBundle] pathForResource:
                          @"XZPointParticleShader" ofType:@"fsh"];
    ...
    //位置
    glBindAttribLocation(program, XZParticleEmissionPosition,
                         "a_emissionPosition");
    //速度
    glBindAttribLocation(program, XZParticleEmissionVelocity,
                         "a_emissionVelocity");
    //重力
    glBindAttribLocation(program, XZParticleEmissionForce,
                         "a_emissionForce");
    //大小
    glBindAttribLocation(program, XZParticleSize,
                         "a_size");
    //持续时间、渐隐时间
    glBindAttribLocation(program, XZParticleEmissionTimeAndLife,
    ...
    // 获取uniform变量的位置.
    //MVP变换矩阵
    uniforms[XZMVPMatrix] = glGetUniformLocation(program,"u_mvpMatrix");
    //纹理
    uniforms[XZSamplers2D] = glGetUniformLocation(program,"u_samplers2D");
    //重力
    uniforms[XZGravity] = glGetUniformLocation(program,"u_gravity");
    //持续时间、渐隐时间
    uniforms[XZElapsedSeconds] = glGetUniformLocation(program,"u_elapsedSeconds");
...
//默认重力
const GLKVector3 XZDefaultGravity = {0.0f, -9.80665f, 0.0f};
