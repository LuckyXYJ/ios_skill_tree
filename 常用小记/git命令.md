...
```
git remote rename old new # git重命名远程仓库
```
...
git merge 命令，将 branchName 分支合并到当前分支
...
使用 git branch --no-merged 命令，查看未与当前分支合并过的分支。
...
```
 ...
 如果你尝试检出的分支不存在且 刚好只有一个名字与之匹配的远程分支，那么 Git 就会为你创建一个跟踪分支：
