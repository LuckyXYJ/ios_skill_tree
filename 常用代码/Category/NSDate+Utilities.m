...
// Thanks, AshFurrow
static const unsigned componentFlags = (NSCalendarUnitYear| NSCalendarUnitMonth | NSCalendarUnitDay | NSCalendarUnitQuarter |  NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond | NSCalendarUnitWeekday | NSCalendarUnitWeekdayOrdinal);
...
+ (NSDate *)dateWithTimestampString:(NSString *)timestamp {
    
    if (timestamp.length > 0) {
        return [self dateWithTimestamp:[timestamp longLongValue]];
    }
    return nil;
}

+ (NSDate *)dateWithTimestamp:(long)timestamp {
